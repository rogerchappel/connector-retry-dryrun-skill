...
import { planFromLog, renderMarkdown, checkPlan, parseApprovalPolicy } from './index.js';
...
try {
  ...
  else if (cmd === 'check') { const file = process.argv[3]; if (!file) throw new Error('missing plan json'); const plan = JSON.parse(fs.readFileSync(file,'utf8')); const failures = checkPlan(plan, parseApprovalPolicy(arg('--require-approval','risky'))); if (failures.length) { console.error(failures.join('\n')); process.exit(1); } console.log('retry plan check passed'); }
